...
def generate_dataset(n_samples, n_nodes, save_dir):
    capacity = 1000
    service_time = 0.1
    loc = np.random.uniform(low=0, high=0.8,size=(n_samples, n_nodes+1, 2))
    ...
    a = a / 100
    b = b / 100
    dataset = {"loc": (loc*100).astype("int"),
               "demand":demand.astype("int"),
               "start":(a*100).astype("int"),
               "end":(b*100).astype("int"),
               "capacity":capacity,
               "service_time":np.full(n_nodes,service_time*100).astype("int")}
    # print('dataset:', dataset)
    # loc = np.squeeze( (loc * 100).astype("int"), axis=(1,1) )
    # print('loc:',loc)
    # demand = np.squeeze( demand.astype("int"), axis=(1,1) )
    # start = np.squeeze( (a * 100).astype("int"), axis=(1,1) )
    # end = np.squeeze( (b * 100).astype("int"), axis=(1,1) )
    # service_time = np.full(n_nodes, service_time * 100).astype("int")

    loc = np.squeeze( (loc * 100).astype("int"))
    demand = demand.astype("int")
    start =  (a * 100).astype("int")
    end =   (b * 100).astype("int")
    service_time = np.full(n_nodes, service_time * 100).astype("int")
    ...
    # print('cust_no',cust_no)

    # nodes = np.concatenate((loc, demand, start, end, service_time))
    cust_no = np.arange(1, n_nodes + 1)
    nodes = np.concatenate((cust_no.reshape(-1, 1),loc[1:, :], demand.T, start.T, end.T, service_time.reshape(-1, 1)), axis=1)
    # Create the final array in the desired format
    # nodes = [b.tolist(), [], []]
    np.savetxt('nodes.txt', nodes, fmt='%d')
    # loc[0, 0] = 40
    # loc[0, 1] = 40
    plt.scatter(loc[1:, 0], loc[1:, 1], s=2)  # plot all points except the first one
    plt.scatter(loc[0, 0], loc[0, 1], marker='*', color='red', s=40)  # plot the first point as a red *
    ...
    if save_dir == "CVRPTW_test":
        ...
        # with open(save_dir + "/cvrptw_" + str(n_nodes) + ".txt", ) as f:
        #     pickle.dump(dataset, f)
        return
